#!/bin/bash
################################################################################
# HICO-DET Action Referring Task Evaluation Script for Claude API
...
#
# Usage:
#   bash run_hico_action_referring_eval_claude.sh [MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage with Sonnet 4.5
#   ANTHROPIC_API_KEY=xxx bash run_hico_action_referring_eval_claude.sh
#
#   # With extended thinking
#   EXTENDED_THINKING=1 bash run_hico_action_referring_eval_claude.sh
#
#   # Quick test on first 10 triplets
#   MAX_IMAGES=10 bash run_hico_action_referring_eval_claude.sh
#
#   # With BERTScore computation
#   COMPUTE_BERTSCORE=1 bash run_hico_action_referring_eval_claude.sh
#
#   # Use different model
#   bash run_hico_action_referring_eval_claude.sh "claude-opus-4.5-20250514"
#
#   # Combine multiple flags
#   VERBOSE=1 MAX_IMAGES=10 EXTENDED_THINKING=1 COMPUTE_BERTSCORE=1 bash run_hico_action_referring_eval_claude.sh
#
...
#   WANDB_PROJECT         W&B project name (default: hico-action-referring-claude)
...
#
# Output files:
#   {output_dir}/hico_action_claude_results_{timestamp}.json          # Raw predictions
#   {output_dir}/hico_action_claude_results_{timestamp}_metrics.json  # Metrics
#   {output_dir}/hico_action_claude_results_{timestamp}_bertscore.json # BERTScore (if computed)
#   {output_dir}/hico_action_claude_results_{timestamp}_thinking.jsonl # Thinking content (if enabled)
#   {output_dir}/hico_action_claude_results_{timestamp}_per_sample.json # Per-sample details (VERBOSE)
#   {output_dir}/hico_action_claude_evaluation_{timestamp}.log        # Full log
################################################################################
...
OUTPUT_DIR="${2:-results/hico_action_claude}"
...
LOG_FILE="$OUTPUT_DIR/hico_action_claude_evaluation_${TIMESTAMP}.log"
...
PRED_FILE="${OUTPUT_DIR}/hico_action_claude_results_${TIMESTAMP}.json"

echo "========================================================================"
echo "HICO-DET Action Referring Evaluation (Claude API)"
echo "========================================================================"
...
if [ ! -z "$WANDB" ]; then
    ...
    if [ ! -z "$WANDB_PROJECT" ]; then
        ...
        echo "  WandB project: hico-action-referring-claude (default)"
    fi
...
EVAL_CMD="python3 eval_hico_action_referring_claude.py \
