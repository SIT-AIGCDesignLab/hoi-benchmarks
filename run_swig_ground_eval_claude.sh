#!/bin/bash
################################################################################
# SWIG-HOI Grounding Task Evaluation Script for Claude API
...
#
# Usage:
#   bash run_swig_ground_eval_claude.sh [MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage with Sonnet 4.5
#   ANTHROPIC_API_KEY=xxx bash run_swig_ground_eval_claude.sh
#
#   # With extended thinking
#   EXTENDED_THINKING=1 bash run_swig_ground_eval_claude.sh
#
#   # Quick test on first 10 images
#   MAX_IMAGES=10 bash run_swig_ground_eval_claude.sh
#
#   # Use different model
#   bash run_swig_ground_eval_claude.sh "claude-opus-4.5-20250514"
#
#   # Combine multiple flags
#   VERBOSE=1 MAX_IMAGES=10 EXTENDED_THINKING=1 bash run_swig_ground_eval_claude.sh
#
...
#   WANDB_PROJECT         W&B project name (default: swig-grounding-claude)
...
#
# Output files:
#   {output_dir}/swig_ground_claude_results_{timestamp}.json          # Raw predictions
#   {output_dir}/swig_ground_claude_results_{timestamp}_metrics.json  # AR metrics
#   {output_dir}/swig_ground_claude_results_{timestamp}_thinking.jsonl # Thinking content (if enabled)
#   {output_dir}/swig_ground_claude_evaluation_{timestamp}.log        # Full log
################################################################################
...
OUTPUT_DIR="${2:-results/swig_ground_claude}"
...
LOG_FILE="$OUTPUT_DIR/swig_ground_claude_evaluation_${TIMESTAMP}.log"

# SWIG dataset paths
# Use environment variable if set, otherwise use default
SWIG_ROOT="${SWIG_ROOT:-data/swig_hoi}"
IMG_PREFIX="${IMG_PREFIX:-${SWIG_ROOT}/images_512}"
ANN_FILE="${ANN_FILE:-data/benchmarks_simplified/swig_ground_test_simplified.json}"
RESULT_FILE="${OUTPUT_DIR}/swig_ground_claude_results_${TIMESTAMP}.json"

echo "========================================================================"
echo "SWIG-HOI Grounding Evaluation (Claude API)"
echo "========================================================================"
...
if [ ! -z "$WANDB" ]; then
    ...
    if [ ! -z "$WANDB_PROJECT" ]; then
        ...
        echo "  WandB project: swig-grounding-claude (default)"
    fi
...
EVAL_CMD="python3 eval_swig_ground_claude.py \
