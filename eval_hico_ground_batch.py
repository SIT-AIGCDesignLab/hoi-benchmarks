#!/usr/bin/env python3
"""
HICO-DET Grounding Batch Evaluation Script
...
Usage:
    python eval_hico_ground_batch.py \
        --provider claude \
        ...
        --output-dir results/hico_ground_claude_batch \
        --resume
"""
...
def build_grounding_prompt(provider: str, optimized: bool = False) -> str:
    """
    Build prompt for HICO grounding task - provider-specific to match real-time scripts.
...
def prepare_batch_requests(
    ...
    for idx, sample in enumerate(tqdm(samples, desc="Preparing requests")):
        ...
        # gt_box_inds contains [person1_idx, object1_idx, person2_idx, object2_idx, ...]
        if len(boxes) >= 2 and len(gt_box_inds) >= 2:
            for i in range(num_pairs):
                if i * 2 + 1 < len(gt_box_inds):
...
def compute_grounding_metrics(
    ...
    # AR over [0.5, 0.95] with size-based tracking
...
def main():
    parser = argparse.ArgumentParser(
        description="HICO-DET Grounding Batch Evaluation"
    )
    ...
    print("\n" + "=" * 80)
    print("HICO-DET Grounding Batch Evaluation")
    print("=" * 80)
    ...
    if args.resume:
        incomplete = checkpoint_mgr.find_incomplete(provider=args.provider, task="hico_ground")
        if incomplete:
    ...
    if job is None:
        ...
        processed_ids = set()
        if args.resume:
            latest = checkpoint_mgr.find_latest(provider=args.provider, task="hico_ground")
            if latest:
        ...
        display_name = f"hico_ground_{args.provider}_{timestamp}"
    ...
    print("\n" + "=" * 80)
    print("HICO-DET Grounding Results (Batch)")
    print("=" * 80)
