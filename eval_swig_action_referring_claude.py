#!/usr/bin/env python3
"""
SWIG-HOI Action Referring Evaluation Script for Claude API

Evaluates Claude models (Sonnet 4.5, Opus 4.5, etc.) on action prediction
using METEOR, CIDEr, BLEU, ROUGE-L, and BERTScore metrics.
...
Usage:
    python eval_swig_action_referring_claude.py \
...
def call_claude_api(
    ...
    """
    Call Claude API with image and bounding boxes.
...
def build_action_referring_prompt(extended_thinking: bool = False, optimized: bool = False) -> str:
    """
    Build prompt for action referring task (Claude-optimized with XML structure).
    ...
    if optimized:
        ...
        return (
            "<role>You are an expert in human-object interaction recognition with deep understanding of visual semantics.</role>\n\n"
            ...
            "</visual_guide>\n\n"
            "<analysis_steps>\n"
            "1. Examine the person's pose and body position\n"
            "2. Identify the object they're interacting with\n"
            "3. Determine the specific action based on visual evidence\n"
            "</analysis_steps>\n\n"
            "<output_requirements>\n"
            "- Use present participle form (verb ending in -ing)\n"
            "- Include object name: [action-ing] [object]\n"
            ...
            "- boarding bus\n"
            "</examples>\n\n"
            "Output: Provide ONLY the action phrase (no quotes, no explanation)."
        )
    else:
        return (
...
def main():
    parser = argparse.ArgumentParser(
        description="SWIG-HOI Action Referring Evaluation with Claude API"
    )
    ...
    parser.add_argument('--wandb-project', type=str, default='swig-action-referring-claude',
    ...
    if use_wandb:
        run_name = args.wandb_run_name or f"claude_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        wandb.init(
    ...
    print("\n" + "=" * 80)
    print("SWIG-HOI Action Referring Evaluation (Claude API)")
    print("=" * 80)
    ...
    # Build prompt (use optimized version to save tokens)
    # Note: Optimized prompt is enabled by default to reduce costs
    # Use optimized prompt if enabled and not using extended thinking
    use_optimized_prompt = args.optimized_prompts and not args.extended_thinking
    prompt_text = build_action_referring_prompt(args.extended_thinking, optimized=use_optimized_prompt)
    ...
    for idx, sample in enumerate(tqdm(dataset_samples)):
        ...
        if pred_text is None:
            ...
            # Call Claude API
            try:
                pred_text, thinking_content = call_claude_api(
    ...
    # Print results
    print("\n" + "=" * 80)
    print("SWIG-HOI Action Referring Results (Claude API)")
    print("=" * 80)
