#!/usr/bin/env python3
"""
HICO-DET Grounding Evaluation Script for OpenAI API

Evaluates OpenAI models on grounding performance with COCO-style AR metrics.
Supports GPT-4o, GPT-4-turbo, and o1 reasoning models.

Usage:
    python eval_hico_ground_openai.py \
        --model-name gpt-4o \
...
def parse_json_response(response_text: str, img_width: int, img_height: int) -> List[Dict]:
    """Parse JSON response to extract person-object pairs in pixel coordinates."""
    pairs = []
    ...
    try:
        ...
        if isinstance(parsed, dict):
            detections = parsed.get('pairs', parsed.get('detections', parsed.get('results', [parsed])))
        ...
        if array_matches:
            try:
                parsed = json.loads(max(array_matches, key=len))
    ...
    for det in detections:
        ...
        if "person_bbox" in det and "object_bbox" in det:
            ...
            if len(person_bbox_norm) == 4 and len(object_bbox_norm) == 4:
                pairs.append({
                    'person_box': denormalize_bbox_from_1000(person_bbox_norm, img_width, img_height),
                    'object_box': denormalize_bbox_from_1000(object_bbox_norm, img_width, img_height),
...
def call_openai_api_grounding(
    ...
    """Call OpenAI API for grounding task."""
    if not OPENAI_AVAILABLE:
        raise ImportError("openai package required. Install with: pip install openai")

    # Get API key (check both OPEN_AI_API_KEY and OPENAI_API_KEY)
    api_key = api_key or os.environ.get('OPEN_AI_API_KEY') or os.environ.get('OPENAI_API_KEY')
    if not api_key:
        raise ValueError("API key not found. Set OPEN_AI_API_KEY or OPENAI_API_KEY or pass --api-key")
    ...
    def _api_call():
        if "o1" in model.lower() or "o3" in model.lower():
            # o1/o3 reasoning models - use default settings
            return client.chat.completions.create(model=model, messages=messages)
        elif 'gpt-5' in model:
            # GPT-5.x models support reasoning_effort - medium balances speed and accuracy
            return client.chat.completions.create(
                model=model,
                messages=messages,
                max_completion_tokens=4096,
                temperature=1.0,
                reasoning_effort="medium"  # Medium effort for reasonable speed/accuracy tradeoff
            )
        else:
            return client.chat.completions.create(model=model, messages=messages, max_tokens=4096, temperature=0.0)

    response = retry_with_exponential_backoff(_api_call)

    reasoning_content = getattr(response.choices[0].message, 'reasoning', '') or ""
    response_text = response.choices[0].message.content or ""

    predicted_pairs = parse_json_response(response_text, img_width, img_height)

    return predicted_pairs, reasoning_content


def build_grounding_prompt(is_o1_model: bool = False, optimized: bool = False) -> str:
    """
    Build prompt for grounding task (OpenAI-optimized with examples).

    Args:
        is_o1_model: Whether this is for o1 reasoning model
    ...
    """
    if optimized:
        # Optimized prompt for OpenAI: concise with clear example
        return (
        ...
        return (
            "Identify ALL person-object pairs performing the specified action.\n\n"
            "CRITICAL: Bounding boxes must be TIGHT around actual objects.\n"
            "- person_bbox: should only cover the person, not the entire scene\n"
            "- object_bbox: should only cover the target object\n\n"
            "Output JSON: [{\"pair_id\": 1, \"person_bbox\": [x1,y1,x2,y2], \"object_bbox\": [x1,y1,x2,y2]}]\n"
            "Use [0-1000] normalized coordinates. Return [] if no pairs."
        )
    else:
        return (
...
def match_pairs_greedy(pred_pairs, gt_pairs, iou_threshold=0.5):
    """Match predicted pairs to ground truth using greedy matching."""
    matches = []
    matched_preds = set()
    matched_gts = set()

    iou_matrix = [[
        (calculate_iou(p['person_box'], g['person_box']) + calculate_iou(p['object_box'], g['object_box'])) / 2.0
        if calculate_iou(p['person_box'], g['person_box']) >= iou_threshold and calculate_iou(p['object_box'], g['object_box']) >= iou_threshold
        else 0.0
        for g in gt_pairs
    ] for p in pred_pairs]

    while True:
        best_score, best_pred, best_gt = 0.0, -1, -1
        for p_idx in range(len(pred_pairs)):
            if p_idx in matched_preds: continue
            for g_idx in range(len(gt_pairs)):
                if g_idx in matched_gts: continue
                if iou_matrix[p_idx][g_idx] > best_score:
                    best_score, best_pred, best_gt = iou_matrix[p_idx][g_idx], p_idx, g_idx
        if best_score == 0.0: break
        matches.append((best_pred, best_gt))
...
def main():
    parser = argparse.ArgumentParser(description="HICO-DET Grounding Evaluation with OpenAI API")
    ...
    parser.add_argument('--wandb-project', type=str, default='hico-grounding-openai')
    ...
    parser.add_argument('--rate-limit', type=int, default=None,
                        help='Max requests per minute (default: 500 for OpenAI)')
    ...
    if use_wandb:
        wandb.init(project=args.wandb_project, name=args.wandb_run_name or f"openai_ground_{datetime.now().strftime('%Y%m%d_%H%M%S')}")

    print("\n" + "=" * 80)
    print("HICO-DET Grounding Evaluation (OpenAI API)")
    print("=" * 80)
    ...
    # Handle simplified format (list) or original format (dict)
    if isinstance(annotations, dict):
        dataset_samples = annotations.get('samples', annotations.get('data', annotations))
    else:
    ...
    if args.concurrent_requests > 1:
        ...
        if args.rate_limit is None:
            args.rate_limit = 500  # OpenAI default
    ...
    prompt_text = build_grounding_prompt(is_o1_model, optimized=use_optimized_prompt)
    ...
    for idx, sample in enumerate(tqdm(dataset_samples)):
        ...
        if pred_pairs is None:
            ...
            # Call OpenAI API
            try:
                pred_pairs, reasoning = call_openai_api_grounding(
    ...
    all_metrics = {'model': args.model_name, 'is_o1_model': is_o1_model, 'total_samples': len(results), 'timestamp': datetime.now().strftime('%Y%m%d_%H%M%S'), **metrics}
    save_results(results, args.result_file, all_metrics, reasoning_contents if reasoning_contents else None)
