#!/bin/bash
################################################################################
# SWIG-HOI Action Referring Task Evaluation Script for Gemini API
...
#
# Usage:
#   bash run_swig_action_referring_eval_gemini.sh [MODEL] [OUTPUT_DIR]
#
# Examples:
#   # Basic usage with Gemini 2.0 Flash
#   GOOGLE_API_KEY=xxx bash run_swig_action_referring_eval_gemini.sh
#
#   # Quick test on first 10 triplets
#   MAX_IMAGES=10 bash run_swig_action_referring_eval_gemini.sh
#
#   # With BERTScore computation
#   COMPUTE_BERTSCORE=1 bash run_swig_action_referring_eval_gemini.sh
#
#   # Use different model
#   bash run_swig_action_referring_eval_gemini.sh "gemini-2.5-pro"
#
...
OUTPUT_DIR="results/swig_action_gemini"
    ...
    case $1 in
        ...
        *)
            ...
            if [ "$MODEL_NAME" = "gemini-2.0-flash-exp" ]; then
                MODEL_NAME="$1"
                shift
            elif [ "$OUTPUT_DIR" = "results/swig_action_gemini" ]; then
                OUTPUT_DIR="$1"
                shift
            else
                shift
...
LOG_FILE="$OUTPUT_DIR/swig_action_gemini_evaluation_${TIMESTAMP}.log"
...
PRED_FILE="${OUTPUT_DIR}/swig_action_gemini_results_${TIMESTAMP}.json"

echo "========================================================================"
echo "SWIG-HOI Action Referring Evaluation (Gemini API)"
echo "========================================================================"
...
EVAL_CMD="python3 eval_swig_action_referring_gemini.py \
